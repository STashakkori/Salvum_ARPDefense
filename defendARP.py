...
def startDefense(ipAddress, my_ip, interface, isTest):
    '''
    ...
    while monitor == True:
        ...
        if goodMac != mac:
            ...
            attackersIP = ''
            # print(type(mac))
            attackersIP = getAttackerIP(ipAddress, str(mac,"UTF-8"))
...
def getMyIp(interface):
    ...
    if (type(interface) != type("")):
      interface_str = str(interface,'UTF-8')
      
    p = subprocess.Popen("ifconfig " + interface_str + " | grep 'inet addr' | awk -F: '{print $2}' | awk '{print $1}'", shell=True, stdout=subprocess.PIPE)
    output = p.communicate()[0].rstrip()
    try:
        output_str = str(output,'UTF-8')
        socket.inet_aton(output_str)
        return output
    except socket.error:
        return ''
...
def getAttackerIP(goodIP, mac):
    ...
    #output = p.communicate()[0].rstrip()
    output = str(p.communicate()[0].rstrip(),'UTF-8')
...
def main(argv):
    ...
    startDefense(options.ip_addr, my_ip, interface, monitorOption)
